...
function loadNavbar() {
    if (app.authorizationService.getCredentials() && app.authorizationService.getRole() === 'ADMIN') {
        app.templateLoader.loadTemplate('.navbar-holder', 'navbar-admin');
    } else if (app.authorizationService.getCredentials() && app.getRole() === 'USER') {
    ...
    app.templateLoader.loadTemplate('.app', 'login', function () {
        $('#login-user').click(function (e) {
            ...
            $.ajax({
                ...
                let authorizationToken = data.split('Bearer ')[1];
                app.authorizationService.saveCredentials(authorizationToken);
...
app.router.on("#/users/register", null, function () {
    app.templateLoader.loadTemplate('.app', 'register', function () {
        $('#register-user').click(function () {
            ...
            }).done((data) => window.location.href = '#/users/login').fail((err) => console.log(err));
        })
    });
});
    ...
    $.ajax({
        ...
        app.templateLoader.loadTemplate('.app', 'users-all', function () {
            ...
            for (let elem of data) {
                ...
                if (elem['role'] === 'USER') {
                    $('#' + actionsId).append('<h5><button class="btn btn-primary promote-btn">Promote</button></h5>');
                } else if (elem['role'] === 'ADMIN') {
                    $('#' + actionsId).append('<h5><button class="btn btn-danger demote-btn">Demote</button></h5>');
                } else {
                    $('#' + actionsId)
                        .append('<h5><button class="btn btn-primary promote-btn">Promote</button></h5>')
                        .append('<h5><button class="btn btn-danger demote-btn">Demote</button></h5>');
                }

                $('#' + actionsId + '>h5>.promote-btn').click(function (e) {
                ...
                $('#' + actionsId + '>h5>.demote-btn').click(function (e) {
                    console.log(actionsId);
